...
from models.utils.custom_types import DrivingExample, DrivingInput, DrivingLabel, LanguageLabel
from models.utils.internvl2_utils import preprocess_image_batch, get_custom_chat_template, get_num_image_tokens_per_patch
from models.utils.projection import get_camera_intrinsics, get_camera_extrinsics
...
class DataModule(LightningDataModule):
    def __init__(
        ...
        **cfg,
        ...
        self.prompt_styles = ['Aggressive', 'Normal', 'Conservative']
        self.printed = False

        self.NUM_IMAGE_PATCHES = 2
        self.IMAGES_TO_CONSIDER = ['image_ff']
        # taken from:
    ...
    def setup(self, stage=None):
        if not self.predict:
                ...
                for udd_i, (used_driving_dataset, used_train_partitions) in enumerate(zip(used_driving_datasets, used_train_partitions)):
                    ...
                    for bucket in bucket_list_tmp:
                        for prompt_style in self.prompt_styles:
                            self.cfg['prompt_style'] = prompt_style
                            datasets[bucket+'_'+prompt_style] = hydra.utils.instantiate(
            ...
            if len(datasets) > 0:
                ...
                if self.prompt_styles is not None:
                    sample_weights = [sample_weights[i] for i, bucket in enumerate(bucket_list) for prompt_style in self.prompt_styles]
                    bucket_list = [bucket+'_'+prompt_style for bucket in bucket_list for prompt_style in self.prompt_styles]
                ...
                assert num_buckets > 0, "No active buckets"
                target_per_bucket = DESIRED_TOTAL // num_buckets  # integer floor

                # Deterministic subsampling (change seed if you like)
                ...
                for b in bucket_list:
                    ds = datasets[b]
                    L = len(ds)
                    # Take at most target_per_bucket from this bucket
                    k = min(target_per_bucket, L)
                    if k < L:
                        # randomly choose k unique indices from this bucket
                        idx = torch.randperm(L, generator=g)[:k].tolist()
                        datasets[b] = torch.utils.data.Subset(ds, idx)
                    # If L < target_per_bucket, we keep L (we won't fabricate new unique items)
    ...
    def dl_collate_fn(self, data):
        ...
        return DrivingExample(
            ...
            bbs_other_actors=[data[i].bbs_other_actors for i in range(BS)],
            route_global=[data[i].route_global for i in range(BS)],
            current_ego_action=[data[i].current_ego_action for i in range(BS)],
            current_ego_position=[data[i].current_ego_position for i in range(BS)],
            current_ego_yaw=[data[i].current_ego_yaw for i in range(BS)],
            current_ego_speed=[data[i].current_ego_speed for i in range(BS)],
            ego_extent=[data[i].ego_extent for i in range(BS)],
            prompt=[data[i].prompt for i in range(BS)],
            speed_limit=[data[i].speed_limit for i in range(BS)],
            target_speed=[data[i].target_speed for i in range(BS)],
            scenario_type=[data[i].scenario_type for i in range(BS)],
            style=[data[i].style for i in range(BS)],
        )
