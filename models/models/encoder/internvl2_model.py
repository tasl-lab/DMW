    ...
    def replace_placeholder_tokens(
        ...
        if inputs_embeds is None:
            ...
            if special_ids.size(0) > 0 and len(placeholder_values) > 0:
                ...
                coords = [torch.tensor(placeholder_values[b_id][special_ids[key_id].item()], device=input_ids.device, dtype=wp_encoder_dtype) for key_id, b_id in zip(special_token_pos[:, 1], special_token_pos[:, 0])] # [[ 20.0000,  -5.8438],  [ 46.7500, -10.6250]]
                coords_length_org = [len(coord) for coord in coords] # [2, 2, 2, 2]

                coords = torch.cat(coords) # [8, 2]
                wp_embeds = wp_encoder(coords.unsqueeze(0)).squeeze(0) # [8, 896]
                wp_embeds = torch.split(wp_embeds, coords_length_org) # [[2, 896], [2, 896], [2, 896], [2, 896]]
