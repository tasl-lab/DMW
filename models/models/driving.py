...
from models.models.adaptors.adaptors import DrivingAdaptor, LanguageAdaptor, WaypointInputAdaptor, UserProfileAdaptor, AdaptorList
from models.models.utils import summarise_losses
from models.utils.custom_types import (DrivingExample, DrivingInput,
...
class DrivingModel(pl.LightningModule):
    def __init__(
        ...
        **cfg,
        ...
        driving = DrivingAdaptor(
            ...
            use_user_profile=self.use_user_profile,
        )
        
        if self.use_user_profile:
            default_dtype = torch.get_default_dtype()
            torch.set_default_dtype(torch.float32)
            self.user_profile = UserProfileAdaptor(
                user_model_path=os.environ.get('USER_PROFILE_MODEL_PATH', 'checkpoints/user_profile_model.pth'),
    ...
    def forward(self,
        ...
        rl_mode: Optional[bool] = False,
        deterministic: Optional[bool] = False,
    ) -> DrivingOutput:
        """
        Samples a trajectory from the model.
        """
        self.speed_wps, self.route, self.language, self.driving_features, self.driving_logits, self.dreaming_dist, self.dreaming_logits, self.dreaming_action_idx, self.dreaming_action_log_prob, self.dreaming_action_entropy, self.dreaming_action_str = None, None, [], None, None, None, None, None, None, None, None
        try:
        ...
        self.predict_language = return_language
        if self.predict_language:  
            ...
            for b_idx, (input_embed, attention_mask) in enumerate(zip(input_embeds_all, attention_masks)):
                ...
                self.driving_features.append(features[:, -len_driving:])
                self.driving_logits.append(logits[:, -len_driving:])

                predictions = self.adaptors.driving.get_predictions(self.driving_features[-1], self.driving_logits[-1])
            ...
            outputs_by_adaptor_features = self.adaptors.split_outputs_by_adaptor(adaptor_dict, features)
            outputs_by_adaptor_logits = self.adaptors.split_outputs_by_adaptor(adaptor_dict, logits)

            self.driving_features = outputs_by_adaptor_features['driving']
            self.driving_logits = outputs_by_adaptor_logits['driving']

            predictions = self.adaptors.driving.get_predictions(self.driving_features, self.driving_logits, rl_mode=rl_mode, deterministic=deterministic)
        ...
        if rl_mode:
            return self.speed_wps, self.route, self.language, self.dreaming_actions, self.dreaming_action_log_prob, self.dreaming_action_entropy
        else:
    ...
    def predict_step(self, batch: DrivingExample, _batch_idx: int = 0):
        ...
        speed_wps, route, language = self.forward(batch, return_language=False)
        ...
        return speed_wps, route, language, driving_features, driving_logits, speed_wps_gt, route_gt, language_gt
    ...
    def on_predict_epoch_end(self) -> None:    
        ...
        for samples, name in zip([samples_safety, samples_instruction, samples_neither, samples_all], ["instruction"]):
            ...
            for i in range(len(samples)):
                ...
                # get desired speed from waypoints
                ...
                # fit linear slope to speed profiles
